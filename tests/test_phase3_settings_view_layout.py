...
@pytest.mark.asyncio
async def test_settings_calendar_button_opens_modal(repo):
    repo.ensure_settings(1, "Guild")
    bot = SimpleNamespace(
        repo=repo,
        _get_guild_feature_settings=lambda _guild_id: _feature_settings(),
    ...
    view = SettingsView(bot, guild_id=1)
    button = next(item for item in view.children if getattr(item, "custom_id", "") == "settings:1:calendar_modal")
    ...
    assert send_modal.await_count == 1
